...
: node-tree-init { depth -- node }
  node% %alloc { node }
  depth if
    depth 1- recurse node node-left !
    depth 1- recurse node node-right !
  else
    0 node node-left !
    0 node node-right !
  then
...
: node-tree-deinit ( node -- )
  dup 0= if drop exit then
  dup node-left @ recurse
  dup node-right @ recurse
  free throw
;
...
: main ( -- )
  ...
  long-lived-tree node-tree-deinit
