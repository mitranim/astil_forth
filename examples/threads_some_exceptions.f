...
\
\ This version of the example has a lax / sloppy approach to error handling:
\ implicit exceptions are fine, accidental explicit errors are fine, and can
\ be checked if they appear. In all cases we maintain full ABI compatibility
\ with foreign C code.
\
...
\ The exception is returned to `main` when joining with this thread and
\ used to find out if the child has succeeded or failed.
\
